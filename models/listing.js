...
let listsch = new Schema({
  ...
  category:{
    type:String,
    required:true,
    enum:['mountain','trending','adventure','river','peace']
  }
})

listsch.post("findOneAndDelete",async (data)=>{
  if(data){
    await review.deleteMany({ _id : { $in: data.reviews}})
  }
})

let listing= mongoose.model("listing",listsch)
...
let sampleListings = [
    {Escape
      ...
      description:
        " to this charming beachfront cottage for a relaxing getaway. Enjoy stunning ocean views and easy access to the beach.",
      image: 
...
sampleListings.forEach((ele)=>{           
    ele.owner='678aaac585911a51dac2521b'
  })
